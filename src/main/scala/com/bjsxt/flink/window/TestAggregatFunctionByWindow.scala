package com.bjsxt.flink.window
...
object TestAggregatFunctionByWindow {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //读取数据源
    val stream: DataStream[StationLog] = streamEnv.socketTextStream("10.172.246.227",8888)
      .map(line=>{
        var arr=line.split(",")
  ...
  class MyAggregateFunction extends AggregateFunction[(String,Int),Long,Long]{
    ...
    override def add(value: (String, Int), accumulator: Long): Long = {
      val add = accumulator + value._2
      println(value._1 + ":" + add)
      add
    }
